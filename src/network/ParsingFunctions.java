...
public class ParsingFunctions {
	...
	public static Message packetFlow(Message msg) {
		...
		try {
			...
			if(!(msg.getDestination().equals(ParsingFunctions.myUUID))){
				//nowhere to relay to, routing concept broken
				throw new Exception("Throwing away packet, not destined to me");
			}else{
				...
				case "04":
					...
					if(msg.getFlag().equals("01")){
						//return new InitializeAuth(msg.getByteData());
						//NetworkCore.authSequene.add(new SuccessAuth(myUUID, msg.getSource()));
						Router.routingTable.add(new Route(msg.getSource(), msg.getSource(), 1));
